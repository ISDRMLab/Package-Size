...
struct ImgRestore::Ximpl
{
	...
	void drawPerpectivePoints();//画透射变换的四个点
...
vector<Point> ImgRestore::Ximpl::axisSort(const vector<Vec4i>& lines)
{
	...
	/*for (auto i : points)
	cout << i.x << " " << i.y << endl;*/
	sort(points.begin(), points.end(), CmpDistanceToZero());//根据到原点的距离从小到大排列
	return points;
}
void ImgRestore::Ximpl::doEdgeDetect()
{
	...
	for (size_t i = 0; i < this->lines.size(); i++)
	{
		...
		line(this->edgeDetect, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(0, 60, 255), 1, CV_AA);//画直线
	}
...
vector<Point> ImgRestore::Ximpl::findCrossPoint(const vector<vector<Point>>& contours)//通过轮廓找点
{
...
void ImgRestore::Ximpl::drawPerpectivePoints()
{
	...
	imshow("透射变换的四个点[原图]", this->srcImage);
...
void ImgRestore::Ximpl::doAffineTransform()
{
	...
	drawPerpectivePoints();//将这四个点在原图中画出来
	...
	this->dstImage = Mat::zeros(newHeight, newWidth, this->srcImage.type());//构建结果图像
	//构建透射变换矩阵 H
	...
	Mat status;
	Mat h = findHomography(m1, m2, status, 0, 3);//构建H矩阵
	perspectiveTransform(srcTriangle, dstTriangle, h); //对二维或者三维矢量进行透射变换
	warpPerspective(this->srcImage, this->dstImage, h, this->dstImage.size());//对图像进行透射变换
